...
C
         ELSE
          WRITE(*,*) '* Execute flow calculation before derivatives'
        ENDIF
C
C------------------------------------------------------
...
C
        ENDIF
C
C------ LUOUT is the handle for the file that has been opened.
...
C
C------------------------------------------------------
      ELSE IF(COMAND.EQ.'WW  ') THEN
C------ write all force data to a file
        CALL STRIP(FNOUT,NFN)
        WRITE(*,1081) FNOUT(1:NFN)
 1081   FORMAT('Enter forces output file: ', A)
        READ (*,1000) FNNEW
C
        IF(FNNEW.NE.' ') THEN
C-------- new filename was entered...
...
C
      DO IV = 1, NVTOT
        ...
        CHSS = '  '
        DO JV = 1, NVTOT
          IF(IV.NE.JV .AND. ICON(IV,IRUN).EQ.ICON(JV,IRUN)) THEN
           CHSS = '**'
          ENDIF
        ENDDO
        WRITE(*,1020) VARKEY(IV), CONNAM(IC), CONVAL(IC,IRUN), CHSS
      ENDDO
C
      WRITE(*,1030)
      RETURN
