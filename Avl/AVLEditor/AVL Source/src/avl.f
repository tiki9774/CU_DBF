...
C
      ENDIF
C
C See if the user has provided a path for an AeroML file, and write
C to it if necessary
C-------------------------------------------------------------------
      CALL GETARG0(4, AMLFIL)
      IF(AMLFIL.NE.' ') THEN
        CALL STRIP(AMLFIL, LENAML)
        DO Q = 1, NRUN
          INFO = 1
          CALL EXEC(NITMAX, INFO, Q)
          CALL AEROMLREAD(Q)
        ENDDO
        CALL AEROMLWRITE(AMLFIL)
        GOTO 1051
      ENDIF
C
C-------------------------------------------------------------------
