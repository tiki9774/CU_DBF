%% AirfoilCompare2.m created by: Kevin Bieri
...
%% create score vector for CL_max
% want to maximize
CLmaxs = ceil(10*tiedrank(CLmax)/length(CLmax));
...
score = zeros(r,1);
for n = 1:r
    score(n) = Ds(n)*wD + LDs(n)*wLD + Cms(n)*wCm + CLmaxs(n)*wCLmax;
    if isnan(score(n))
        score(n) = 0;
    end
end
...
% write header
header = 'Filename  Score  S*cd_c  Dc  L/D_c  LDc  Cm  Cmc  CL_max  CLs';
fprintf(fid,'%s\n',header);
