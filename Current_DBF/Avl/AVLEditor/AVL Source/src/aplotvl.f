...
C---- start geometry plot
    6 CALL PVLINI(TITLE,AZIM,ELEV,TILT,VERSION)
C
...
C
        ELSEIF(INDEX('Ii',CKEY).NE.0) THEN
         IF(RINV.EQ.0.0) THEN
          RINV = 0.02/BREF
          WRITE(*,*) 'Viewpoint brought in from infinity'
         ELSE
          RINV = RINV*PRAT
         ENDIF
         GO TO 4
C
        ELSEIF(INDEX('Oo',CKEY).NE.0) THEN
         IF(RINV .LT. 0.02/BREF) THEN
          RINV = 0.0
          WRITE(*,*) 'Viewpoint set at infinity (no perspective)'
         ELSE
          RINV = RINV/PRAT
         ENDIF
         GO TO 4
...
C----- Display options 
 50    CONTINUE
     ...
     &              LABEL_SURF, LABEL_STRP, LABEL_VRTX
 5500  FORMAT(/' ================================================'
...
C
      WRITE(*,*) ' '
      WRITE(*,*) '==================================================='
      WRITE(*,*) '#surf  index  Nchord Nspan   plot?   Surface name'
      DO N = 1, NSURF
        WRITE(*,61) N, LSURF(N), NK(N), NJ(N), LPLTSURF(N), STITLE(N)
      END DO
...
C---- small axis arrows
      ...
      DO L = 1, 3
        AORG = SQRT(ORG(1,L)**2 + ORG(2,L)**2)
        IF(AORG.EQ.0.0) AORG = 1.0
C
        CALL PLOT(XO,YO,3)   
        CALL PLOT(XO+DLEN*ORG(1,L),YO+DLEN*ORG(2,L),2)   
C
        XPLT = XO + ORG(1,L)*(DLEN + SCH/AORG) - 0.5*SCH
        YPLT = YO + ORG(2,L)*(DLEN + SCH/AORG) - 0.5*SCH
        CALL PLCHAR(XPLT,YPLT,0.9*SCH,XYZ(L),0.0,1)
      ENDDO
      CALL NEWCOLOR(ICOL0)
C
