...
C
      CALL SLEN(PROMPT,NP)
      NP = MAX( NP , 1 )
      IF(INDEX('^',PROMPT(NP:NP)) .NE. 0) NP = NP-1
      NP = MAX( NP , 1 )
C
...
ccc   WRITE(*,*) 'GETFLT: String-to-integer conversion error.'
      ...
      SUBROUTINE GETLGV(INPUT,A,N,ERROR)
      CHARACTER*(*) INPUT
      LOGICAL A(*)
      LOGICAL ERROR
C----------------------------------------------------------
...
C     in INPUT will be extracted.
C
C     N returns how many variables were actually extracted.
C----------------------------------------------------------
      CHARACTER*130 REC
C
C---- only first 128 characters in INPUT will be parsed
...
C
      NINP = N
C
C---- count up how many variables are to be extracted
      N = 0
      K = 1
      DO 10 IPASS=1, ILEN
C------ search for next space or comma starting with current index K
...
C
 ...
 10   CONTINUE
C
C---- decide on how many variables to read, and go ahead and read them
 11   IF(NINP.GT.0) N = MIN( N, NINP )
      READ(REC(1:ILEN),*,ERR=20) (A(I),I=1,N)
      ERROR = .FALSE.
      RETURN
...
C---- bzzzt !!!
 20   CONTINUE
ccc   WRITE(*,*) 'GETLGV: String-to-logical conversion error.'
      N = 0
      ERROR = .TRUE.
      RETURN
      END
...
C
      ...
      SUBROUTINE SLEN(STRING,NS)
      CHARACTER*(*) STRING
C-------------------------------------------------
C     Returns length of non-blank part of string
C-------------------------------------------------
      N = LEN(STRING)
C
C---- find last non-blank character
