...
delta_cf = 2*0.0063; % linear density of cf tube with Do = 0.625in and Di = 0.5in
 % length of tail boom inside tail cone [in]

% calculate tail moment arm
fuse_length = cr*0.75; % length of fuselage from c/4 to tail boom
...
weight_boom = delta_cf*(L); % weight of tail boom [lb]
